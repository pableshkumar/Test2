...
import getStatus from "@salesforce/apex/VendorOnBoardingController.getAccountData";
...
export default class InputPicklistField extends LightningElement {
  ...
  controllingValue;
  status;
  accOption=[{ label: "--None--", value: "", selected: true }];
  @api label;
  @api name;
  @api value;
  @api objectApiName;
  ...
  setUpPicklistValues(data) {
    ...
    if (data.picklistFieldValues[this.fieldApiName]) {
      if (this.isDependentPicklist) {
        ...
        if (
          data.picklistFieldValues[this.fieldApiName].controllerValues !==
          undefined
        ) {
          ...
          Object.entries(controllerValues).forEach(([key, value]) => {
            const picValues =
              data.picklistFieldValues[this.fieldApiName].values;
